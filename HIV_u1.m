...
[t_K, y_K] = ode15s(@(t,y)Model_HIV_Control(t,y,tforward,u3),tforward,initial_cond,options);
figure
plot(t_H,y_H(:,1),'-.','LineWidth',2.5)
 hold on
plot(t_O,y_O(:,1),'r--','LineWidth',2.5)
plot(t_K,y_K(:,1),'g','LineWidth',2.5)
legend('with u_1=0 ','with u1=1', 'with u1=100','location','northwest') 
...
plot(t_H,y_H(:,2),'-.','LineWidth',2.5)
 hold on
plot(t_O,y_O(:,2),'--','LineWidth',2.5)
plot(t_K,y_K(:,2),'g','LineWidth',2.5)
legend('with u1=0 ','with u1=1','with u1=100', 'Location','northwest') 
 title('Number of opioid addicted Individuals when u_1 only is present','fontweight','normal','fontsize',18)
 ylabel('U(t)','fontweight','normal','fontsize',18)
 xlabel('Years','fontweight','normal','fontsize',18)
 xticks([1 6 11 16 21 26 31 36 40 ])
...
% legend('with u1=0','with u1=100','Location','northwest') 
...
[t_r, y_r] = ode15s(@(t,y)Model_HIV_0(y),tforward,initial_cond);
figure 
...
kbj= y(:,2);
kbj1=y_r(:,2);
plot(t,kbj,'g','LineWidth',2.5)
 hold on
plot(t,kbj1,'--','LineWidth',2.5)
...
legend('with controls','with out controls','location','northwest') 
 title('Number of addicted with first stage infected Individuals','fontweight','normal','fontsize',12)
 ylabel('I_1(t)','fontweight','normal','fontsize',18)
 xlabel('Years','fontweight','normal','fontsize',18)
 xticks([1 6 11 16 21 26 31 36 40 ])
 set(gca, 'YGrid', 'on', 'XGrid', 'off','LineWidth', 2,'fontsize',13)
hold off
% 
figure
...
legend('with controls','with out controls','location','northwest') 
 title('Number of addicted and second stage infected Individuals','fontweight','normal','fontsize',10)
 ylabel('I_2(t)','fontweight','normal','fontsize',18)
 xlabel('Years','fontweight','normal','fontsize',18)
 xticks([1 6 11 16 21 26 31 36 40 ])
 set(gca, 'YGrid', 'on', 'XGrid', 'off','LineWidth', 2,'fontsize',13)
...
title('Treatment for Addicted Individuals only (u_1)','fontweight','normal','fontsize',18)
...
dy(2) = (beta_u*(U+I_1+I_2)*S./N)-(q_u*beta_v*(V+I_1+I_2)*U./N)- (mu+mu_u + delta*u1t)*U;
...
u1timedep = utimedep(:,1);
   ...
   dlambda(3) = -1 + ((lambda(2)-lambda(1))*beta_u*St*(Ut+I_1t+I_2t)./Nt^2)+...
   ...
   dlambda(5) = -1 + ((lambda(1)-lambda(2))* beta_u*St* (St+Vt+At)./Nt^2) +...
              ...
              ((lambda(2)-lambda(4))*q_u*beta_v*Ut*(St+Ut+At)./Nt^2)+...
              ((lambda(3)-lambda(4))*q_v*beta_u*Vt* (St+Vt+At)./Nt^2)+...
              ((lambda(5)-lambda(6))*gamma_2)+...
              (lambda(5)*(mu+mu_2))+...
              ((lambda(5)-lambda(3))*delta_2);  
   dlambda(6) = -1 + ((lambda(2)-lambda(1))*beta_u*St*(Ut+I_1t+I_2t)./Nt^2)+...
               ((lambda(3)-lambda(1))*beta_v*St*(Vt+I_1t+I_2t)./Nt^2)+...
               ((lambda(4)-lambda(2))*q_u*beta_v*(Vt+I_1t+I_2t)*Ut./Nt^2)+...
               ((lambda(4)-lambda(3))*q_v*beta_u*(Ut+I_1t+I_2t)*Vt./Nt^2)+...
               (lambda(6)*(mu+mu_a));
...
N  = y(1) + y(2) + y(3) + y(4) + y(5)+ y(6);
   ...
   dy(1) = Lambda - beta_u*(U+I_1+I_2)*S./N - beta_v*(V+I_1+I_2)*S./N- mu*S ;   
   dy(2) = beta_u*(U+I_1+I_2)*S./N - q_u*beta_v*(V+I_1+I_2)*U./N - (mu + mu_u )*U;
   dy(3) = beta_v*(V+I_1+I_2)*S./N - q_v*beta_u*(U+I_1+I_2)*V./N - (mu + mu_v + gamma_v)*V + delta_2*I_2;
   dy(4) = q_u*beta_v*(V+I_1+I_2)*U./N + q_v*beta_u*(U+I_1+I_2)*V./N - (mu + alpha + mu_1)*I_1;
   dy(5) = alpha*I_1 - (mu + gamma_2+ mu_2 + delta_2)*I_2;
   dy(6) = gamma_v*V + gamma_2*I_2 - (mu + mu_a)*A;
