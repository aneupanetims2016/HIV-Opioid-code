...
plot(t_H,y_H(:,5),'-.','LineWidth',2.5)
 hold on
plot(t_O,y_O(:,5),'r--','LineWidth',2.5)
plot(t_K,y_K(:,5),'g','LineWidth',2.5)
legend('with u_4=0 ','with u_4=1','with u_4=100','Location','northwest') 
 title('Number of opioid addicted second stage infected (I_2) with u_4 only','fontweight','normal','fontsize',18)
...
legend('with u_4=0 ','with u_4=50','Location','northwest') 
...
legend('with u_4=0 ','with u_4=100') 
...
plot(t_K,y_K(:,4),'r--','LineWidth',2.5)
legend('with u4=0','with u4=1','with u4=100','Location','northwest') 
...
plot(t_K,y_K(:,5),'r--','LineWidth',2.5)
legend('with u_4=0 ','with u_4=1','with u_4=100','Location','northwest') 
...
plot(t_K,y_K(:,6),'r--','LineWidth',2.5)
legend('with u_4=0','with u_4=1','with u_4=100') 
...
legend('with control u_4','with out control u_4','location','northwest') 
...
legend('with control u_4','with out control u_4','location','northwest') 
 ...
 yticklabels({'2x10^4', '4x10^4', '6x10^4', '8x10^4', '10x10^4','12x10^4'})
 set(gca, 'YGrid', 'on', 'XGrid', 'off','LineWidth', 2,'fontsize',14)
hold off
% 
figure
...
legend('with controls','with out controls') 
 ...
 yticklabels({'1x10^5', '2x10^5', '3x10^5', '4x10^5', '5x10^5'})
set(gca, 'YGrid', 'on', 'XGrid', 'off','LineWidth', 2,'fontsize',14)
hold off

figure
plot(tforward,u(:,1),'r-.','LineWidth',2.5)
title('Treatment for addicted-infected  individuals only (u_4)','fontweight','normal','fontsize',18)
...
dy(3) = (beta_v*(V+I_1+I_2)*S./N)-(q_v*beta_u*(U+I_1+I_2)*V./N)- (mu+mu_v+gamma_v)*V+ delta_2*u4t*I_2;
dy(4) =  (q_u*beta_v*(V+I_1+I_2)*U./N) + (q_v*beta_u*(U+I_1+I_2)*V./N)-(mu+alpha+mu_1)*I_1;
dy(5) = alpha*I_1-(mu+gamma_2+mu_2 + delta_2*u4t)*I_2;
...
N  = y(1) + y(2) + y(3) + y(4) + y(5)+ y(6);
   ...
   dy(1) = Lambda - beta_u*(U+I_1+I_2)*S./N - beta_v*(V+I_1+I_2)*S./N- mu*S + delta*U;   
   dy(2) = beta_u*(U+I_1+I_2)*S./N - q_u*beta_v*(V+I_1+I_2)*U./N - (mu + mu_u + delta)*U;
   dy(3) = beta_v*(V+I_1+I_2)*S./N - q_v*beta_u*(U+I_1+I_2)*V./N - (mu + mu_v + gamma_v)*V ;
   dy(4) = q_u*beta_v*(V+I_1+I_2)*U./N + q_v*beta_u*(U+I_1+I_2)*V./N - (mu + alpha + mu_1)*I_1;
   dy(5) = alpha*I_1 - (mu + gamma_2+ mu_2 )*I_2;
   dy(6) = gamma_v*V + gamma_2*I_2 - (mu + mu_a)*A;
